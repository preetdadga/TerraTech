"""Routes for TerraTech web application."""
...
def create_routes(app):
    ...
    def predict():
        try:
            ...
            plt.tight_layout()
            graph_path = UPLOAD_FOLDER / "forecast.png"
            ...
            # Weekly recommendations - group by weeks and use averages
            farmer_seller_recommendations = []
            consumer_recommendations = []
            government_recommendations = []
            weekly_data = []

            # Group forecast into weeks
            for week_start in range(0, len(forecast_table), 7):
                week_end = min(week_start + 7, len(forecast_table))
                week_values = [forecast_table[i][1] for i in range(week_start, week_end)]
                week_dates = [forecast_table[i][0] for i in range(week_start, week_end)]
                week_avg = sum(week_values) / len(week_values)
                
                # Get previous week average for comparison
                if week_start == 0:
                    prev_week_avg = df['value'].iloc[-1]  # Use last known value as baseline
                else:
                    prev_week_avg = weekly_data[-1]['avg']
                
                change = week_avg - prev_week_avg

                week_label = f"Week {len(weekly_data) + 1}"
                
                if change > 0.8:
                    farmer_seller_recommendations.append({
                        "week": week_label,
                        "dates": f"{week_dates[0]} to {week_dates[-1]}",
                        "avg_price": round(week_avg, 2),
                        "farmer_seller_action": "Hold your crop, price is increasing."
                    })
                    consumer_recommendations.append({
                        "week": week_label,
                        "consumer_action": "Buy now before prices rise more."
                    })
                    government_recommendations.append({
                        "week": week_label,
                        "government_action": "Monitor for inflation; consider price caps."
                    })
                elif change < -0.8:
                    farmer_seller_recommendations.append({
                        "week": week_label,
                        "dates": f"{week_dates[0]} to {week_dates[-1]}",
                        "avg_price": round(week_avg, 2),
                        "farmer_seller_action": "Consider selling before price drops further."
                    })
                    consumer_recommendations.append({
                        "week": week_label,
                        "consumer_action": "Delay buying; prices may go down."
                    })
                    government_recommendations.append({
                        "week": week_label,
                        "government_action": "No major action; ensure fair pricing."
                    })
                else:
                    farmer_seller_recommendations.append({
                        "week": week_label,
                        "dates": f"{week_dates[0]} to {week_dates[-1]}",
                        "avg_price": round(week_avg, 2),
                        "farmer_seller_action": "Stable prices; act as usual."
                    })
                    consumer_recommendations.append({
                        "week": week_label,
                        "consumer_action": "Stable market; buy as needed."
                    })
                    government_recommendations.append({
                        "week": week_label,
                        "government_action": "Maintain monitoring; no action required."
                    })
    ...
    def download_data():
        try:
            ...
            return render_template("index.html", error=f"Download failed: {str(e)}", current_year=datetime.now().year)